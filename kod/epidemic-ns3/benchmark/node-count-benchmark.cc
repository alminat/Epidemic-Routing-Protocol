#include "node-count-benchmark.h"
#include <fstream>

NodeCountBenchmark::NodeCountBenchmark(
...
void NodeCountBenchmark::run()
{
    Gnuplot2dDataset dataset;
    dataset.SetTitle("Delivery Ratio %");
    dataset.SetStyle(Gnuplot2dDataset::LINES_POINTS);

    std::ofstream csvFile;
    csvFile.open("treci_scenarij_broj_cvorova.csv", std::ios::out);

    csvFile << "nWifis,queueLength,hopCount,benchmark,parameter,sent,received,lost,packet_loss,delivery_ratio\n";

    for (uint32_t currentNodes = minNodes; currentNodes <= maxNodes; currentNodes += nodeStep)
    {
        std::string tag = "nWifis: " + std::to_string(currentNodes);
        ...
        nodeContainer.Create(currentNodes);
...
// PCAP snimanje WiFi saobraćaja za Wireshark analizu
std::string pcapName = "node-count-" + std::to_string(currentNodes);
wifiPhy.EnablePcapAll(pcapName);
        ...
        uint32_t num_source = currentNodes;
        uint32_t num_sink = currentNodes;
        ...
        for (uint32_t i = 0; i < num_sink; ++i)
        {
            ...
            pktSink->TraceConnect("Rx", ss.str(),
                                  MakeCallback(&NodeCountBenchmark::SinkRxTrace, this));
        ...
        dataset.Add(currentNodes, deliveryRatio);

        csvFile << currentNodes << ","
                << queueLength << ","
                << hopCount << ","
                << "NodeCount" << ","
                << currentNodes << ","
...
std::ostream &operator<<(std::ostream &os, NodeCountBenchmark &b)
{
    operator<<(os, (Benchmark &)b);
    os << "Min nodes: " << b.minNodes << std::endl;
    os << "Max nodes: " << b.maxNodes << std::endl;
    os << "Node step: " << b.nodeStep << std::endl;
    return os;
}
